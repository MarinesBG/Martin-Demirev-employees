...
namespace Employee.Services.Services
{
    public class WorkCalculationService : IWorkCalculationService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var pairResults = new Dictionary<(int EmpA, int EmpB), List<ProjectDetail>>();
            ...
            foreach (var projectGroup in projectGroups)
            {
                ...
                for (int i = 0; i < employeesOnProject.Count; i++)
                {
                    ...
                    {
                        var emp1 = employeesOnProject[i];
                        var emp2 = employeesOnProject[j];
                        ...
                        if (overlapStart <= overlapEnd)
                        {
                            ...
                            if (dateDifference < 0 || dateDifference > 36500) // 100 years max
                            {
                                throw new InvalidOperationException(
                                    $"Invalid date range for employees {emp1.EmpID} and {emp2.EmpID} on project {projectId}: " +
                            ...
                            var pairKey = emp1.EmpID < emp2.EmpID 
                                ? (emp1.EmpID, emp2.EmpID) 
                                : (emp2.EmpID, emp1.EmpID);

                            _logger?.LogDebug(
                                "Pair ({EmpA},{EmpB}) on Project {ProjectId}: {Days} days ({Start} to {End})",
                        ...
                        {
                            _logger?.LogDebug(
                                "No overlap: Emp {Emp1} ({From1} to {To1}) and Emp {Emp2} ({From2} to {To2}) on Project {ProjectId}",
                                emp1.EmpID, emp1.DateFrom.ToString("yyyy-MM-dd"), emp1.DateTo.ToString("yyyy-MM-dd"),
                                emp2.EmpID, emp2.DateFrom.ToString("yyyy-MM-dd"), emp2.DateTo.ToString("yyyy-MM-dd"),
                                projectId);
                        }
                    }
                }
            ...
            {
                var (empA, empB) = kvp.Key;
                var totalDays = kvp.Value.Sum(p => p.DaysWorked);
                
                _logger?.LogInformation(
                    "Pair ({EmpA},{EmpB}): {TotalDays} days across {ProjectCount} projects",
                    empA, empB, totalDays, kvp.Value.Count);
            ...
            _logger?.LogInformation(
                "TOP PAIR: ({EmpA},{EmpB}) worked together for {TotalDays} days",
        ...
        public Task<IEnumerable<PairResult>> GetLastComputedPairsAsync(CancellationToken cancellationToken = default)
        {
            return Task.FromResult<IEnumerable<PairResult>>(_lastComputedPairs);
        }

        public Task<PairResult?> GetPairDetailsAsync(int empA, int empB, CancellationToken cancellationToken = default)
        {
            // Normalize the pair (always smaller ID first)
            var (smallerId, largerId) = empA < empB ? (empA, empB) : (empB, empA);

            var pair = _lastComputedPairs.FirstOrDefault(p => 
                p.EmployeeIdA == smallerId && p.EmployeeIdB == largerId);
